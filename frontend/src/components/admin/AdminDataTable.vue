...
<template>
  <div class="overflow-x-auto border border-slate-200/80 dark:border-slate-800/80 rounded-xl">
    <table class="w-full text-sm">
      <thead class="bg-slate-50/80 dark:bg-slate-800/50 backdrop-blur-sm">
        <tr>
          ...
          <th v-if="actions?.length" class="px-6 py-4 text-right text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
            操作
          </th>
        </tr>
      </thead>
    ...
    <div v-if="!loading && data.length === 0" class="px-6 py-8 text-center text-sm text-slate-500 dark:text-slate-400">
      {{ emptyText || '暂无数据' }}
    </div>
    <div v-if="loading" class="px-6 py-8 text-center text-sm text-slate-500 dark:text-slate-400 animate-pulse">
      加载中...
    </div>
  </div>
</template>
