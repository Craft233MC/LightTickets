...
function onHsvInput(channel: 'h' | 's' | 'v', e: Event) {
  ...
  const n = parseInt(raw)
  if (isNaN(n)) return
  if (channel === 'h') { hue.value = Math.max(0, Math.min(360, n)); syncFromHsv() }
  else if (channel === 's') { sat.value = Math.max(0, Math.min(100, n)) / 100; syncFromHsv() }
  else { val.value = Math.max(0, Math.min(100, n)) / 100; syncFromHsv() }
}

// --- Preset colors ---
const presets = [
  '#ef4444', '#f97316', '#eab308', '#22c55e', '#14b8a6', '#3b82f6',
  '#6366f1', '#a855f7', '#ec4899', '#78716c', '#64748b', '#000000',
]
</script>
  ...
  <div class="space-y-3">
    ...
    <div class="grid grid-cols-2 gap-2 text-xs">
      <div class="flex items-center gap-1">
        ...
        <input :value="rInput" @input="onRgbInput('r', $event)" class="w-full px-2 py-1 rounded border border-slate-300 dark:border-slate-700 bg-white/80 dark:bg-slate-900/80 text-slate-900 dark:text-slate-100 font-mono" />
      </div>
      <div class="flex items-center gap-1">
        <span class="w-3 text-slate-500 text-right">H</span>
        <input :value="hInput" @input="onHsvInput('h', $event)" class="w-full px-2 py-1 rounded border border-slate-300 dark:border-slate-700 bg-white/80 dark:bg-slate-900/80 text-slate-900 dark:text-slate-100 font-mono" />
      </div>
      <div class="flex items-center gap-1">
        <span class="w-3 text-slate-500 text-right">G</span>
        <input :value="gInput" @input="onRgbInput('g', $event)" class="w-full px-2 py-1 rounded border border-slate-300 dark:border-slate-700 bg-white/80 dark:bg-slate-900/80 text-slate-900 dark:text-slate-100 font-mono" />
      </div>
      <div class="flex items-center gap-1">
        <span class="w-3 text-slate-500 text-right">S</span>
        <input :value="sInput" @input="onHsvInput('s', $event)" class="w-full px-2 py-1 rounded border border-slate-300 dark:border-slate-700 bg-white/80 dark:bg-slate-900/80 text-slate-900 dark:text-slate-100 font-mono" />
      </div>
      <div class="flex items-center gap-1">
        <span class="w-3 text-slate-500 text-right">B</span>
        <input :value="bInput" @input="onRgbInput('b', $event)" class="w-full px-2 py-1 rounded border border-slate-300 dark:border-slate-700 bg-white/80 dark:bg-slate-900/80 text-slate-900 dark:text-slate-100 font-mono" />
      </div>
      <div class="flex items-center gap-1">
        <span class="w-3 text-slate-500 text-right">V</span>
        <input :value="vInput" @input="onHsvInput('v', $event)" class="w-full px-2 py-1 rounded border border-slate-300 dark:border-slate-700 bg-white/80 dark:bg-slate-900/80 text-slate-900 dark:text-slate-100 font-mono" />
      </div>
    </div>
